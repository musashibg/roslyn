...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal enum BoundKind: byte
    {
        ...
        AssignmentOperator,
        LocalDeconstructionDeclaration,
        DeconstructionAssignmentOperator,
        DeconstructionDeconstructStep,
        DeconstructionAssignmentStep,
        Void,
    ...
    {
        protected BoundValuePlaceholderBase(BoundKind kind, CSharpSyntaxNode syntax, TypeSymbol type, bool hasErrors)
        ...
        protected BoundValuePlaceholderBase(BoundKind kind, CSharpSyntaxNode syntax, TypeSymbol type)
    ...
    {
        public BoundDeconstructValuePlaceholder(CSharpSyntaxNode syntax, TypeSymbol type, bool hasErrors)
        ...
        public BoundDeconstructValuePlaceholder(CSharpSyntaxNode syntax, TypeSymbol type)
    ...
    {
        public BoundLocalDeconstructionDeclaration(CSharpSyntaxNode syntax, BoundDeconstructionAssignmentOperator assignment, bool hasErrors = false)
            : base(BoundKind.LocalDeconstructionDeclaration, syntax, hasErrors || assignment.HasErrors())
        {

            Debug.Assert(assignment != null, "Field 'assignment' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");

            this.Assignment = assignment;
        }


        public BoundDeconstructionAssignmentOperator Assignment { get; }

        public override BoundNode Accept(BoundTreeVisitor visitor)
        {
            return visitor.VisitLocalDeconstructionDeclaration(this);
        }

        public BoundLocalDeconstructionDeclaration Update(BoundDeconstructionAssignmentOperator assignment)
        {
            if (assignment != this.Assignment)
            {
                var result = new BoundLocalDeconstructionDeclaration(this.Syntax, assignment, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public BoundDeconstructionAssignmentOperator(CSharpSyntaxNode syntax, bool isDeclaration, ImmutableArray<BoundExpression> leftVariables, BoundExpression right, ImmutableArray<BoundDeconstructionDeconstructStep> deconstructSteps, ImmutableArray<BoundDeconstructionAssignmentStep> assignmentSteps, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.DeconstructionAssignmentOperator, syntax, type, hasErrors || leftVariables.HasErrors() || right.HasErrors() || deconstructSteps.HasErrors() || assignmentSteps.HasErrors())
        {

            Debug.Assert(!leftVariables.IsDefault, "Field 'leftVariables' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            Debug.Assert(right != null, "Field 'right' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            Debug.Assert(type != null, "Field 'type' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            ...
            this.Right = right;
            this.DeconstructSteps = deconstructSteps;
        ...
        public ImmutableArray<BoundExpression> LeftVariables { get; }

        public BoundExpression Right { get; }

        public ImmutableArray<BoundDeconstructionDeconstructStep> DeconstructSteps { get; }

        public ImmutableArray<BoundDeconstructionAssignmentStep> AssignmentSteps { get; }
        ...
        public BoundDeconstructionAssignmentOperator Update(bool isDeclaration, ImmutableArray<BoundExpression> leftVariables, BoundExpression right, ImmutableArray<BoundDeconstructionDeconstructStep> deconstructSteps, ImmutableArray<BoundDeconstructionAssignmentStep> assignmentSteps, TypeSymbol type)
        {
            if (isDeclaration != this.IsDeclaration || leftVariables != this.LeftVariables || right != this.Right || deconstructSteps != this.DeconstructSteps || assignmentSteps != this.AssignmentSteps || type != this.Type)
            {
                var result = new BoundDeconstructionAssignmentOperator(this.Syntax, isDeclaration, leftVariables, right, deconstructSteps, assignmentSteps, type, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    internal sealed partial class BoundDeconstructionDeconstructStep : BoundNode
    {
        public BoundDeconstructionDeconstructStep(CSharpSyntaxNode syntax, BoundExpression deconstructInvocationOpt, BoundDeconstructValuePlaceholder targetPlaceholder, ImmutableArray<BoundDeconstructValuePlaceholder> outputPlaceholders, bool hasErrors = false)
            : base(BoundKind.DeconstructionDeconstructStep, syntax, hasErrors || deconstructInvocationOpt.HasErrors() || targetPlaceholder.HasErrors() || outputPlaceholders.HasErrors())
        {
            ...
            Debug.Assert(!outputPlaceholders.IsDefault, "Field 'outputPlaceholders' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");

            this.DeconstructInvocationOpt = deconstructInvocationOpt;
            this.TargetPlaceholder = targetPlaceholder;
            this.OutputPlaceholders = outputPlaceholders;
        }


        public BoundExpression DeconstructInvocationOpt { get; }

        public BoundDeconstructValuePlaceholder TargetPlaceholder { get; }

        public ImmutableArray<BoundDeconstructValuePlaceholder> OutputPlaceholders { get; }

        public override BoundNode Accept(BoundTreeVisitor visitor)
        {
            return visitor.VisitDeconstructionDeconstructStep(this);
        }

        public BoundDeconstructionDeconstructStep Update(BoundExpression deconstructInvocationOpt, BoundDeconstructValuePlaceholder targetPlaceholder, ImmutableArray<BoundDeconstructValuePlaceholder> outputPlaceholders)
        {
            if (deconstructInvocationOpt != this.DeconstructInvocationOpt || targetPlaceholder != this.TargetPlaceholder || outputPlaceholders != this.OutputPlaceholders)
            {
                var result = new BoundDeconstructionDeconstructStep(this.Syntax, deconstructInvocationOpt, targetPlaceholder, outputPlaceholders, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    internal sealed partial class BoundDeconstructionAssignmentStep : BoundNode
    {
        public BoundDeconstructionAssignmentStep(CSharpSyntaxNode syntax, BoundAssignmentOperator assignment, BoundDeconstructValuePlaceholder targetPlaceholder, BoundDeconstructValuePlaceholder outputPlaceholder, bool hasErrors = false)
            : base(BoundKind.DeconstructionAssignmentStep, syntax, hasErrors || assignment.HasErrors() || targetPlaceholder.HasErrors() || outputPlaceholder.HasErrors())
        {

            Debug.Assert(assignment != null, "Field 'assignment' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            Debug.Assert(targetPlaceholder != null, "Field 'targetPlaceholder' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            Debug.Assert(outputPlaceholder != null, "Field 'outputPlaceholder' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
        ...
        public BoundAssignmentOperator Assignment { get; }

        public BoundDeconstructValuePlaceholder TargetPlaceholder { get; }

        public BoundDeconstructValuePlaceholder OutputPlaceholder { get; }

        public override BoundNode Accept(BoundTreeVisitor visitor)
        {
            return visitor.VisitDeconstructionAssignmentStep(this);
        }

        public BoundDeconstructionAssignmentStep Update(BoundAssignmentOperator assignment, BoundDeconstructValuePlaceholder targetPlaceholder, BoundDeconstructValuePlaceholder outputPlaceholder)
        {
            if (assignment != this.Assignment || targetPlaceholder != this.TargetPlaceholder || outputPlaceholder != this.OutputPlaceholder)
            {
                var result = new BoundDeconstructionAssignmentStep(this.Syntax, assignment, targetPlaceholder, outputPlaceholder, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public BoundVoid(CSharpSyntaxNode syntax, TypeSymbol type, bool hasErrors)
        ...
        public BoundVoid(CSharpSyntaxNode syntax, TypeSymbol type)
    ...
    {
        public BoundConversion(CSharpSyntaxNode syntax, BoundExpression operand, Conversion conversion, Boolean isBaseConversion, Boolean @checked, Boolean explicitCastInCode, ConstantValue constantValueOpt, TypeSymbol type, bool hasErrors = false)
    ...
    {
        public BoundBlock(CSharpSyntaxNode syntax, ImmutableArray<LocalSymbol> locals, ImmutableArray<LocalFunctionSymbol> localFunctions, ImmutableArray<BoundStatement> statements, bool hasErrors = false)
    ...
    {
        public BoundLocalFunctionStatement(CSharpSyntaxNode syntax, LocalFunctionSymbol symbol, BoundBlock body, bool hasErrors = false)
    ...
    {
        public BoundReturnStatement(CSharpSyntaxNode syntax, RefKind refKind, BoundExpression expressionOpt, bool hasErrors = false)
    ...
    {
        public BoundSwitchSection(CSharpSyntaxNode syntax, ImmutableArray<BoundSwitchLabel> switchLabels, ImmutableArray<BoundStatement> statements, bool hasErrors = false)
    ...
    {
        public BoundSwitchLabel(CSharpSyntaxNode syntax, LabelSymbol label, BoundExpression expressionOpt, ConstantValue constantValueOpt, bool hasErrors = false)
    ...
    {
        public BoundPatternSwitchStatement(CSharpSyntaxNode syntax, BoundExpression expression, ImmutableArray<LocalSymbol> innerLocals, ImmutableArray<LocalFunctionSymbol> innerLocalFunctions, ImmutableArray<BoundPatternSwitchSection> switchSections, BoundPatternSwitchLabel defaultLabel, GeneratedLabelSymbol breakLabel, PatternSwitchBinder binder, bool hasErrors = false)
        ...
        public BoundPatternSwitchStatement Update(BoundExpression expression, ImmutableArray<LocalSymbol> innerLocals, ImmutableArray<LocalFunctionSymbol> innerLocalFunctions, ImmutableArray<BoundPatternSwitchSection> switchSections, BoundPatternSwitchLabel defaultLabel, GeneratedLabelSymbol breakLabel, PatternSwitchBinder binder)
        {
            if (expression != this.Expression || innerLocals != this.InnerLocals || innerLocalFunctions != this.InnerLocalFunctions || switchSections != this.SwitchSections || defaultLabel != this.DefaultLabel || breakLabel != this.BreakLabel || binder != this.Binder)
            {
                var result = new BoundPatternSwitchStatement(this.Syntax, expression, innerLocals, innerLocalFunctions, switchSections, defaultLabel, breakLabel, binder, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public BoundPatternSwitchSection(CSharpSyntaxNode syntax, ImmutableArray<LocalSymbol> locals, ImmutableArray<BoundPatternSwitchLabel> switchLabels, ImmutableArray<BoundStatement> statements, bool hasErrors = false)
    ...
    {
        public BoundPatternSwitchLabel(CSharpSyntaxNode syntax, LabelSymbol label, BoundPattern pattern, BoundExpression guard, bool hasErrors = false)
    ...
    {
        public BoundForEachStatement(CSharpSyntaxNode syntax, ForEachEnumeratorInfo enumeratorInfoOpt, Conversion elementConversion, BoundTypeExpression iterationVariableType, LocalSymbol iterationVariableOpt, BoundExpression expression, BoundForEachDeconstructStep deconstructionOpt, BoundStatement body, Boolean @checked, GeneratedLabelSymbol breakLabel, GeneratedLabelSymbol continueLabel, bool hasErrors = false)
            : base(BoundKind.ForEachStatement, syntax, breakLabel, continueLabel, hasErrors || iterationVariableType.HasErrors() || expression.HasErrors() || deconstructionOpt.HasErrors() || body.HasErrors())
        {
            ...
            this.IterationVariableOpt = iterationVariableOpt;
            this.Expression = expression;
        ...
        public LocalSymbol IterationVariableOpt { get; }
        ...
        public BoundForEachStatement Update(ForEachEnumeratorInfo enumeratorInfoOpt, Conversion elementConversion, BoundTypeExpression iterationVariableType, LocalSymbol iterationVariableOpt, BoundExpression expression, BoundForEachDeconstructStep deconstructionOpt, BoundStatement body, Boolean @checked, GeneratedLabelSymbol breakLabel, GeneratedLabelSymbol continueLabel)
        {
            if (enumeratorInfoOpt != this.EnumeratorInfoOpt || elementConversion != this.ElementConversion || iterationVariableType != this.IterationVariableType || iterationVariableOpt != this.IterationVariableOpt || expression != this.Expression || deconstructionOpt != this.DeconstructionOpt || body != this.Body || @checked != this.Checked || breakLabel != this.BreakLabel || continueLabel != this.ContinueLabel)
            {
                var result = new BoundForEachStatement(this.Syntax, enumeratorInfoOpt, elementConversion, iterationVariableType, iterationVariableOpt, expression, deconstructionOpt, body, @checked, breakLabel, continueLabel, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public BoundForEachDeconstructStep(CSharpSyntaxNode syntax, BoundDeconstructionAssignmentOperator deconstructionAssignment, BoundDeconstructValuePlaceholder targetPlaceholder, bool hasErrors = false)
    ...
    {
        public BoundCatchBlock(CSharpSyntaxNode syntax, ImmutableArray<LocalSymbol> locals, BoundExpression exceptionSourceOpt, TypeSymbol exceptionTypeOpt, BoundExpression exceptionFilterOpt, BoundBlock body, bool isSynthesizedAsyncCatchAll, bool hasErrors = false)
    ...
    {
        protected BoundTupleExpression(BoundKind kind, CSharpSyntaxNode syntax, ImmutableArray<BoundExpression> arguments, TypeSymbol type, bool hasErrors = false)
            : base(kind, syntax, type, hasErrors)
        {
    ...
    {
        public BoundTupleLiteral(CSharpSyntaxNode syntax, ImmutableArray<string> argumentNamesOpt, ImmutableArray<BoundExpression> arguments, TypeSymbol type, bool hasErrors = false)
    ...
    {
        public BoundConvertedTupleLiteral(CSharpSyntaxNode syntax, TypeSymbol naturalTypeOpt, ImmutableArray<BoundExpression> arguments, TypeSymbol type, bool hasErrors = false)
    ...
    {
        public BoundIsPatternExpression(CSharpSyntaxNode syntax, BoundExpression expression, BoundPattern pattern, TypeSymbol type, bool hasErrors = false)
    ...
    {
        protected BoundPattern(BoundKind kind, CSharpSyntaxNode syntax, bool hasErrors)
            : base(kind, syntax, hasErrors)
        {
        }

        protected BoundPattern(BoundKind kind, CSharpSyntaxNode syntax)
            : base(kind, syntax)
        {
        }
    ...
    {
        public BoundDeclarationPattern(CSharpSyntaxNode syntax, LocalSymbol localSymbol, BoundTypeExpression declaredType, bool isVar, bool hasErrors = false)
            : base(BoundKind.DeclarationPattern, syntax, hasErrors || declaredType.HasErrors())
        {
        ...
        public BoundDeclarationPattern Update(LocalSymbol localSymbol, BoundTypeExpression declaredType, bool isVar)
        {
            if (localSymbol != this.LocalSymbol || declaredType != this.DeclaredType || isVar != this.IsVar)
            {
                var result = new BoundDeclarationPattern(this.Syntax, localSymbol, declaredType, isVar, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public BoundConstantPattern(CSharpSyntaxNode syntax, BoundExpression value, ConstantValue constantValue, bool hasErrors = false)
    ...
    {
        public BoundWildcardPattern(CSharpSyntaxNode syntax, bool hasErrors)
            : base(BoundKind.WildcardPattern, syntax, hasErrors)
        {
        }

        public BoundWildcardPattern(CSharpSyntaxNode syntax)
    ...
    internal sealed partial class OutVarLocalPendingInference : BoundExpression
    {
        public OutVarLocalPendingInference(CSharpSyntaxNode syntax, LocalSymbol localSymbol, bool hasErrors)
            : base(BoundKind.OutVarLocalPendingInference, syntax, null, hasErrors)
        {
        ...
        public OutVarLocalPendingInference(CSharpSyntaxNode syntax, LocalSymbol localSymbol)
            : base(BoundKind.OutVarLocalPendingInference, syntax, null)
        {
        ...
        public override BoundNode Accept(BoundTreeVisitor visitor)
        {
            return visitor.VisitOutVarLocalPendingInference(this);
        }

        public OutVarLocalPendingInference Update(LocalSymbol localSymbol)
        {
            if (localSymbol != this.LocalSymbol)
            {
                var result = new OutVarLocalPendingInference(this.Syntax, localSymbol, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    {
        public OutDeconstructVarPendingInference(CSharpSyntaxNode syntax, bool hasErrors)
            : base(BoundKind.OutDeconstructVarPendingInference, syntax, null, hasErrors)
        {
        }

        public OutDeconstructVarPendingInference(CSharpSyntaxNode syntax)
    ...
    internal sealed partial class DeconstructionLocalPendingInference : BoundExpression
    {
        public DeconstructionLocalPendingInference(CSharpSyntaxNode syntax, LocalSymbol localSymbol, bool hasErrors)
            : base(BoundKind.DeconstructionLocalPendingInference, syntax, null, hasErrors)
        {
        ...
        public DeconstructionLocalPendingInference(CSharpSyntaxNode syntax, LocalSymbol localSymbol)
            : base(BoundKind.DeconstructionLocalPendingInference, syntax, null)
        {
        ...
        public override BoundNode Accept(BoundTreeVisitor visitor)
        {
            return visitor.VisitDeconstructionLocalPendingInference(this);
        }

        public DeconstructionLocalPendingInference Update(LocalSymbol localSymbol)
        {
            if (localSymbol != this.LocalSymbol)
            {
                var result = new DeconstructionLocalPendingInference(this.Syntax, localSymbol, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    internal sealed partial class BoundDecorator : BoundExpression
    {
        public BoundDecorator(CSharpSyntaxNode syntax, MethodSymbol constructor, ImmutableArray<BoundExpression> constructorArguments, ImmutableArray<string> constructorArgumentNamesOpt, ImmutableArray<BoundExpression> namedArguments, LookupResultKind resultKind, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.Decorator, syntax, type, hasErrors || constructorArguments.HasErrors() || namedArguments.HasErrors())
        {
        ...
        public BoundDecorator Update(MethodSymbol constructor, ImmutableArray<BoundExpression> constructorArguments, ImmutableArray<string> constructorArgumentNamesOpt, ImmutableArray<BoundExpression> namedArguments, LookupResultKind resultKind, TypeSymbol type)
        {
            if (constructor != this.Constructor || constructorArguments != this.ConstructorArguments || constructorArgumentNamesOpt != this.ConstructorArgumentNamesOpt || namedArguments != this.NamedArguments || resultKind != this.ResultKind || type != this.Type)
            {
                var result = new BoundDecorator(this.Syntax, constructor, constructorArguments, constructorArgumentNamesOpt, namedArguments, resultKind, type, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
    ...
    internal sealed partial class BoundMetaclass : BoundExpression
    {
        public BoundMetaclass(CSharpSyntaxNode syntax, MethodSymbol constructor, ImmutableArray<BoundExpression> constructorArguments, ImmutableArray<string> constructorArgumentNamesOpt, ImmutableArray<BoundExpression> namedArguments, LookupResultKind resultKind, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.Metaclass, syntax, type, hasErrors || constructorArguments.HasErrors() || namedArguments.HasErrors())
        {
        ...
        public BoundMetaclass Update(MethodSymbol constructor, ImmutableArray<BoundExpression> constructorArguments, ImmutableArray<string> constructorArgumentNamesOpt, ImmutableArray<BoundExpression> namedArguments, LookupResultKind resultKind, TypeSymbol type)
        {
            if (constructor != this.Constructor || constructorArguments != this.ConstructorArguments || constructorArgumentNamesOpt != this.ConstructorArgumentNamesOpt || namedArguments != this.NamedArguments || resultKind != this.ResultKind || type != this.Type)
            {
                var result = new BoundMetaclass(this.Syntax, constructor, constructorArguments, constructorArgumentNamesOpt, namedArguments, resultKind, type, this.HasErrors);
                result.WasCompilerGenerated = this.WasCompilerGenerated;
                return result;
            }
            return this;
        ...
        internal R VisitInternal(BoundNode node, A arg)
        {
            switch (node.Kind)
            {
                ...
                case BoundKind.LocalDeconstructionDeclaration: 
                    return VisitLocalDeconstructionDeclaration(node as BoundLocalDeconstructionDeclaration, arg);
                case BoundKind.DeconstructionAssignmentOperator: 
                    return VisitDeconstructionAssignmentOperator(node as BoundDeconstructionAssignmentOperator, arg);
                case BoundKind.DeconstructionDeconstructStep: 
                    return VisitDeconstructionDeconstructStep(node as BoundDeconstructionDeconstructStep, arg);
                case BoundKind.DeconstructionAssignmentStep: 
                    return VisitDeconstructionAssignmentStep(node as BoundDeconstructionAssignmentStep, arg);
                case BoundKind.Void: 
                ...
                case BoundKind.OutVarLocalPendingInference: 
                    return VisitOutVarLocalPendingInference(node as OutVarLocalPendingInference, arg);
                case BoundKind.OutDeconstructVarPendingInference: 
                    return VisitOutDeconstructVarPendingInference(node as OutDeconstructVarPendingInference, arg);
                case BoundKind.DeconstructionLocalPendingInference: 
                    return VisitDeconstructionLocalPendingInference(node as DeconstructionLocalPendingInference, arg);
                case BoundKind.Decorator: 
                    return VisitDecorator(node as BoundDecorator, arg);
                case BoundKind.Metaclass: 
                    return VisitMetaclass(node as BoundMetaclass, arg);
            }
    ...
    internal abstract partial class BoundTreeVisitor<A,R>
    {
        ...
        public virtual R VisitLocalDeconstructionDeclaration(BoundLocalDeconstructionDeclaration node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        ...
        public virtual R VisitDeconstructionDeconstructStep(BoundDeconstructionDeconstructStep node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        public virtual R VisitDeconstructionAssignmentStep(BoundDeconstructionAssignmentStep node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        ...
        public virtual R VisitOutVarLocalPendingInference(OutVarLocalPendingInference node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        ...
        public virtual R VisitDeconstructionLocalPendingInference(DeconstructionLocalPendingInference node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        public virtual R VisitDecorator(BoundDecorator node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
        public virtual R VisitMetaclass(BoundMetaclass node, A arg)
        {
            return this.DefaultVisit(node, arg);
        }
    }
    ...
    internal abstract partial class BoundTreeVisitor
    {
        ...
        public virtual BoundNode VisitLocalDeconstructionDeclaration(BoundLocalDeconstructionDeclaration node)
        {
            return this.DefaultVisit(node);
        }
        ...
        public virtual BoundNode VisitDeconstructionDeconstructStep(BoundDeconstructionDeconstructStep node)
        {
            return this.DefaultVisit(node);
        }
        public virtual BoundNode VisitDeconstructionAssignmentStep(BoundDeconstructionAssignmentStep node)
        {
            return this.DefaultVisit(node);
        }
        ...
        public virtual BoundNode VisitOutVarLocalPendingInference(OutVarLocalPendingInference node)
        {
            return this.DefaultVisit(node);
        }
        ...
        public virtual BoundNode VisitDeconstructionLocalPendingInference(DeconstructionLocalPendingInference node)
        {
            return this.DefaultVisit(node);
        }
        public virtual BoundNode VisitDecorator(BoundDecorator node)
        {
            return this.DefaultVisit(node);
        }
        public virtual BoundNode VisitMetaclass(BoundMetaclass node)
        {
            return this.DefaultVisit(node);
        }
    }
    ...
    internal abstract partial class BoundTreeWalker: BoundTreeVisitor
    {
        ...
        {
            this.VisitList(node.LeftVariables);
            this.Visit(node.Right);
            this.VisitList(node.DeconstructSteps);
            this.VisitList(node.AssignmentSteps);
            return null;
        }
        public override BoundNode VisitDeconstructionDeconstructStep(BoundDeconstructionDeconstructStep node)
        {
            this.Visit(node.DeconstructInvocationOpt);
            this.Visit(node.TargetPlaceholder);
            this.VisitList(node.OutputPlaceholders);
            return null;
        }
        public override BoundNode VisitDeconstructionAssignmentStep(BoundDeconstructionAssignmentStep node)
        {
            this.Visit(node.Assignment);
            this.Visit(node.TargetPlaceholder);
            this.Visit(node.OutputPlaceholder);
            return null;
        }
        ...
        public override BoundNode VisitOutVarLocalPendingInference(OutVarLocalPendingInference node)
        {
            return null;
        }
        ...
        public override BoundNode VisitDecorator(BoundDecorator node)
        {
            this.VisitList(node.ConstructorArguments);
            this.VisitList(node.NamedArguments);
            return null;
        }
        public override BoundNode VisitMetaclass(BoundMetaclass node)
        {
            this.VisitList(node.ConstructorArguments);
            this.VisitList(node.NamedArguments);
            return null;
    ...
    internal abstract partial class BoundTreeRewriter : BoundTreeVisitor
    {
        ...
        public override BoundNode VisitLocalDeconstructionDeclaration(BoundLocalDeconstructionDeclaration node)
        {
            BoundDeconstructionAssignmentOperator assignment = (BoundDeconstructionAssignmentOperator)this.Visit(node.Assignment);
            return node.Update(assignment);
        }
        public override BoundNode VisitDeconstructionAssignmentOperator(BoundDeconstructionAssignmentOperator node)
        {
            ImmutableArray<BoundExpression> leftVariables = (ImmutableArray<BoundExpression>)this.VisitList(node.LeftVariables);
            BoundExpression right = (BoundExpression)this.Visit(node.Right);
            ImmutableArray<BoundDeconstructionDeconstructStep> deconstructSteps = (ImmutableArray<BoundDeconstructionDeconstructStep>)this.VisitList(node.DeconstructSteps);
            ImmutableArray<BoundDeconstructionAssignmentStep> assignmentSteps = (ImmutableArray<BoundDeconstructionAssignmentStep>)this.VisitList(node.AssignmentSteps);
            TypeSymbol type = this.VisitType(node.Type);
            return node.Update(node.IsDeclaration, leftVariables, right, deconstructSteps, assignmentSteps, type);
        }
        public override BoundNode VisitDeconstructionDeconstructStep(BoundDeconstructionDeconstructStep node)
        {
            BoundExpression deconstructInvocationOpt = (BoundExpression)this.Visit(node.DeconstructInvocationOpt);
            BoundDeconstructValuePlaceholder targetPlaceholder = (BoundDeconstructValuePlaceholder)this.Visit(node.TargetPlaceholder);
            ImmutableArray<BoundDeconstructValuePlaceholder> outputPlaceholders = (ImmutableArray<BoundDeconstructValuePlaceholder>)this.VisitList(node.OutputPlaceholders);
            return node.Update(deconstructInvocationOpt, targetPlaceholder, outputPlaceholders);
        }
        public override BoundNode VisitDeconstructionAssignmentStep(BoundDeconstructionAssignmentStep node)
        {
            BoundAssignmentOperator assignment = (BoundAssignmentOperator)this.Visit(node.Assignment);
            BoundDeconstructValuePlaceholder targetPlaceholder = (BoundDeconstructValuePlaceholder)this.Visit(node.TargetPlaceholder);
            BoundDeconstructValuePlaceholder outputPlaceholder = (BoundDeconstructValuePlaceholder)this.Visit(node.OutputPlaceholder);
            return node.Update(assignment, targetPlaceholder, outputPlaceholder);
        }
        public override BoundNode VisitVoid(BoundVoid node)
        {
            TypeSymbol type = this.VisitType(node.Type);
        ...
        public override BoundNode VisitPatternSwitchStatement(BoundPatternSwitchStatement node)
        {
            ...
            return node.Update(expression, node.InnerLocals, node.InnerLocalFunctions, switchSections, defaultLabel, node.BreakLabel, node.Binder);
        }
        public override BoundNode VisitPatternSwitchSection(BoundPatternSwitchSection node)
        {
        ...
        public override BoundNode VisitForEachStatement(BoundForEachStatement node)
        {
            ...
            BoundStatement body = (BoundStatement)this.Visit(node.Body);
            return node.Update(node.EnumeratorInfoOpt, node.ElementConversion, iterationVariableType, node.IterationVariableOpt, expression, deconstructionOpt, body, node.Checked, node.BreakLabel, node.ContinueLabel);
        }
        public override BoundNode VisitForEachDeconstructStep(BoundForEachDeconstructStep node)
        {
        ...
        {
            BoundTypeExpression declaredType = (BoundTypeExpression)this.Visit(node.DeclaredType);
            return node.Update(node.LocalSymbol, declaredType, node.IsVar);
        }
        public override BoundNode VisitConstantPattern(BoundConstantPattern node)
        {
            BoundExpression value = (BoundExpression)this.Visit(node.Value);
        ...
        {
            TypeSymbol type = this.VisitType(node.Type);
            return node.Update(node.LocalSymbol);
        }
        public override BoundNode VisitOutDeconstructVarPendingInference(OutDeconstructVarPendingInference node)
        {
            TypeSymbol type = this.VisitType(node.Type);
    ...
    internal sealed class BoundTreeDumperNodeProducer : BoundTreeVisitor<object, TreeDumperNode>
    {
        ...
        public override TreeDumperNode VisitLocalDeconstructionDeclaration(BoundLocalDeconstructionDeclaration node, object arg)
        {
            return new TreeDumperNode("localDeconstructionDeclaration", null, new TreeDumperNode[]
            {
                new TreeDumperNode("assignment", null, new TreeDumperNode[] { Visit(node.Assignment, null) })
            }
            );
        }
        public override TreeDumperNode VisitDeconstructionAssignmentOperator(BoundDeconstructionAssignmentOperator node, object arg)
        {
            return new TreeDumperNode("deconstructionAssignmentOperator", null, new TreeDumperNode[]
            {
                ...
                new TreeDumperNode("leftVariables", null, from x in node.LeftVariables select Visit(x, null)),
                new TreeDumperNode("right", null, new TreeDumperNode[] { Visit(node.Right, null) }),
                new TreeDumperNode("deconstructSteps", null, from x in node.DeconstructSteps select Visit(x, null)),
                new TreeDumperNode("assignmentSteps", null, from x in node.AssignmentSteps select Visit(x, null)),
                new TreeDumperNode("type", node.Type, null)
            }
            );
        }
        public override TreeDumperNode VisitDeconstructionDeconstructStep(BoundDeconstructionDeconstructStep node, object arg)
        {
            return new TreeDumperNode("deconstructionDeconstructStep", null, new TreeDumperNode[]
            {
                new TreeDumperNode("deconstructInvocationOpt", null, new TreeDumperNode[] { Visit(node.DeconstructInvocationOpt, null) }),
                new TreeDumperNode("targetPlaceholder", null, new TreeDumperNode[] { Visit(node.TargetPlaceholder, null) }),
                new TreeDumperNode("outputPlaceholders", null, from x in node.OutputPlaceholders select Visit(x, null))
            }
            );
        }
        public override TreeDumperNode VisitDeconstructionAssignmentStep(BoundDeconstructionAssignmentStep node, object arg)
        {
            return new TreeDumperNode("deconstructionAssignmentStep", null, new TreeDumperNode[]
            {
                new TreeDumperNode("assignment", null, new TreeDumperNode[] { Visit(node.Assignment, null) }),
                new TreeDumperNode("targetPlaceholder", null, new TreeDumperNode[] { Visit(node.TargetPlaceholder, null) }),
                new TreeDumperNode("outputPlaceholder", null, new TreeDumperNode[] { Visit(node.OutputPlaceholder, null) })
            }
            );
        }
        ...
        public override TreeDumperNode VisitForEachStatement(BoundForEachStatement node, object arg)
        {
            return new TreeDumperNode("forEachStatement", null, new TreeDumperNode[]
            {
                ...
                new TreeDumperNode("iterationVariableOpt", node.IterationVariableOpt, null),
                new TreeDumperNode("expression", null, new TreeDumperNode[] { Visit(node.Expression, null) }),
        ...
        {
            return new TreeDumperNode("wildcardPattern", null, SpecializedCollections.EmptyArray<TreeDumperNode>()
            );
        }
        public override TreeDumperNode VisitOutVarLocalPendingInference(OutVarLocalPendingInference node, object arg)
        {
            return new TreeDumperNode("outVarLocalPendingInference", null, new TreeDumperNode[]
            {
                new TreeDumperNode("localSymbol", node.LocalSymbol, null),
                new TreeDumperNode("type", node.Type, null)
            }
        ...
        public override TreeDumperNode VisitDeconstructionLocalPendingInference(DeconstructionLocalPendingInference node, object arg)
        {
            return new TreeDumperNode("deconstructionLocalPendingInference", null, new TreeDumperNode[]
            {
                new TreeDumperNode("localSymbol", node.LocalSymbol, null),
                new TreeDumperNode("type", node.Type, null)
            }
        ...
        public override TreeDumperNode VisitDecorator(BoundDecorator node, object arg)
        {
            return new TreeDumperNode("decorator", null, new TreeDumperNode[]
            {
                new TreeDumperNode("constructor", node.Constructor, null),
                new TreeDumperNode("constructorArguments", null, from x in node.ConstructorArguments select Visit(x, null)),
                new TreeDumperNode("constructorArgumentNamesOpt", node.ConstructorArgumentNamesOpt, null),
        ...
        public override TreeDumperNode VisitMetaclass(BoundMetaclass node, object arg)
        {
            return new TreeDumperNode("metaclass", null, new TreeDumperNode[]
            {
                new TreeDumperNode("constructor", node.Constructor, null),
                new TreeDumperNode("constructorArguments", null, from x in node.ConstructorArguments select Visit(x, null)),
                new TreeDumperNode("constructorArgumentNamesOpt", node.ConstructorArgumentNamesOpt, null),
